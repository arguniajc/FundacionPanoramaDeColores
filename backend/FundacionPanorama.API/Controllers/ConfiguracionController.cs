...
public class ConfiguracionController : ControllerBase
{
    ...
    // ── Endpoint público (sin auth) — solo datos de la página web pública ──────
    [HttpGet("publica")]
    [AllowAnonymous]
    public async Task<IActionResult> ObtenerPublica()
    {
        await using var conn = AbrirConexion();
        await conn.OpenAsync();
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = @"SELECT nombre_fundacion, email_contacto, sitio_web, footer_texto, web_contenido
        ...
        return Ok(new {
            nombreFundacion = r.IsDBNull(0) ? null : r.GetString(0),
            emailContacto   = r.IsDBNull(1) ? null : r.GetString(1),
            sitioWeb        = r.IsDBNull(2) ? null : r.GetString(2),
            footerTexto     = r.IsDBNull(3) ? null : r.GetString(3),
            webContenido    = r.IsDBNull(4) ? null : r.GetString(4),
        });
    }
