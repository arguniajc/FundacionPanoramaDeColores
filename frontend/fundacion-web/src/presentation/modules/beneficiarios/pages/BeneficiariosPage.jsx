...
// Fila memoizada: solo re-renderiza cuando cambia su propia data o sus callbacks
const BeneficiarioFila = memo(function BeneficiarioFila({
    ...
    >
      ...
      <TableCell sx={{ whiteSpace: 'nowrap', display: { xs: 'none', md: 'table-cell' } }}>
        {ins.whatsapp ? (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <IconButton
              size="small" component="a" title="Abrir en WhatsApp"
      ...
      <TableCell onClick={e => e.stopPropagation()} sx={{ whiteSpace: 'nowrap', pr: 0.5 }}>
        <IconButton size="small" title="Ver detalle"
          sx={{ color: 'var(--color-primario)', '&:hover': { bgcolor: 'rgba(78,27,149,0.1)' } }}
          onClick={() => onVer(ins)}>
...
export default function BeneficiariosPage() {
  ...
  // Debounce search: el TextField usa inputSearch, buscar se actualiza 350ms después
  ...
  // Callbacks estables para las filas (no se recrean en cada render del padre)
